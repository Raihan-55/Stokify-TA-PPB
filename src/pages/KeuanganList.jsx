...
import { getTransaksi, deleteTransaksi, getSummaryBulanan, getSummaryHarian } from "../lib/database";
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <Link to="/keuangan/new" className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition-colors">
          Tambah Transaksi
        </Link>
      </div>
      ...
      <div className="rounded-xl shadow-md bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <div className="flex items-end">
            <button onClick={load} className="w-full px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white rounded-lg font-medium transition-colors">
              Terapkan Filter
            </button>
          </div>
        </div>
        ...
        {list.length === 0 ? (
          ...
          <div className="space-y-3">
            {list.map((t, index) => (
              <div
                key={t.id}
                className="relative flex items-start gap-4 p-4 rounded-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow"
              >
                {/* Timeline indicator */}
                <div className="flex flex-col items-center">
                  <div className={`w-3 h-3 rounded-full ${t.tipe === "pengeluaran" ? "bg-red-500" : "bg-green-500"}`}></div>
                  {index < list.length - 1 && <div className="w-0.5 h-8 bg-gray-300 dark:bg-gray-600 mt-2"></div>}
                </div>
                ...
                <div className="flex gap-2">
                  <Link to={`/keuangan/${t.id}`} className="px-3 py-1 bg-blue-600 hover:bg-blue-700 text-white text-sm rounded-lg transition-colors">
                    Edit
                  </Link>
